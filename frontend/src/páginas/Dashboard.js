...
function Dashboard() {
  ...
  useEffect(() => {
    const carregarDashboard = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/dashboard-real/`, {
        ...
        if (error.response && error.response.status === 401) {
            navigate('/login'); // Se o token expirou, manda para o login
        }
        setLoading(false);
      }
    };
    ...
    return (
      <Layout>
        <div style={{ padding: '50px', textAlign: 'center', fontSize: '18px', color: '#6b7280' }}>
          A carregar o motor da ISO 27001... 🔄
        </div>
      </Layout>
    );
  }
  ...
  // Proteção para o gráfico não "partir" se estiver tudo a zero (conta nova)
  // 1. O total agora inclui o Crítico
  const totalRisco = dadosRisco.critico + dadosRisco.alto + dadosRisco.medio + dadosRisco.baixo;
  
  // 2. Pontos de paragem da "fatia" (Matemática do círculo)
  const fimCritico = dadosRisco.critico;
  const fimAlto = fimCritico + dadosRisco.alto;
  const fimMedio = fimAlto + dadosRisco.medio;

  // 3. Desenha o donut com as 4 cores!
  const graficoBackground = totalRisco === 0 
    ? '#e5e7eb' 
    : `conic-gradient(
        #991b1b 0% ${fimCritico}%, 
        #ef4444 ${fimCritico}% ${fimAlto}%, 
        #f59e0b ${fimAlto}% ${fimMedio}%, 
        #10b981 ${fimMedio}% 100%
      )`;
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px', marginBottom: '30px' }}>
          ...
          <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '12px', border: '1px solid #e5e7eb', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '15px', marginTop: '20px', fontSize: '12px', fontWeight: 'bold' }}>
              <span style={{ color: '#a81414' }}>● Crítico ({dadosRisco.critico}%)</span>
              ...
              <tbody>
                {riscosQuentes.length === 0 ? (
                  ...
                  riscosQuentes.map((r, i) => {
                    // Lógica para a cor bater certo com a página de Riscos
                    let corFundo = '#f59e0b'; // Laranja por defeito (Médio)
                    if (r.nivel === 'Crítico') corFundo = '#991b1b'; // Vermelho Escuro
                    if (r.nivel === 'Alto') corFundo = '#ef4444'; // Vermelho Vivo
                      ...
                      <tr key={i} style={{ borderBottom: '1px solid #f3f4f6' }}>
                        ...
                        <td style={{ padding: '10px' }}>
                          <span style={{ 
                            ...
                            {r.nivel === 'Crítico' ? '🚨 Crítico' : `⚠️ ${r.nivel}`}
                          </span>
                        </td>
          ...
          {meusCursos.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
              {meusCursos.map((curso) => (
                <div key={curso.id_bd} style={{ backgroundColor: 'white', borderRadius: '8px', padding: '15px', border: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                  <div>
                  ...
                  <div>
                    <button 
                      onClick={() => aoClicarNoCurso(curso.id_bd)}
                      style={{
